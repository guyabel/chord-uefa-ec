...
r <- n0 %>%
  ...
  replace_na(list(kit_shirt = "transparent")) %>%
  group_by(lab) %>%
  arrange(year) %>%
  fill(alpha3, .direction = "updown") %>%
  mutate(ease = "linear") %>%
...
pdf(file = "./plot/euro_flex_chord.pdf", useDingbats = FALSE)
...
dev.off()
file.show("plot/euro_flex_chord.pdf")
